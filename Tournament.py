...
class Std_Tournament():
	...
	def play_match(self,p1_indx,p2_indx,rep_type, verifier):
		...
		if(not p1_play and not p2_play):
			...
			self.total_reward[p1_indx]-=penalty
			self.total_reward[p2_indx]+=walkover
			return
		elif(not p2_play):
			self.total_reward[p2_indx]-=penalty
			self.total_reward[p1_indx]+=walkover
			return
